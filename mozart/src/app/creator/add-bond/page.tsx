...
const addBondSchema = z.object({
  ...
  bondPrice: z
    ...
    .refine((val) => val > 0, "Bond price must be positive"),
  expiryDate: z.string(), // Handle date transformation in the onSubmit function
  couponIntervalMonths: z
    .number()
    .min(1, "Coupon interval must be at least 1 month"),
  supplyCap: z
    .string()
    .transform((val) => parseFloat(val))
    .refine((val) => val > 0, "Supply cap must be positive"),
  revenueSharePercentage: z
    .string()
    .transform((val) => parseFloat(val))
    .refine((val) => val > 0, "Revenue share percentage must be positive"),
});
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      const contractFactory = new ethers.ContractFactory(
        revenueShareContractABI,
        revenueShareContractBytecode,
        signer
      );
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col mb-3">
                ...
                <div style={{ maxWidth: "200px" }}>
                  {" "}
                  {/* Set a maximum width here */}
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        className={cn(
                          "inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50",
